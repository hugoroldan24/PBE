...
class Finestra(Gtk.Window):        #Clase relacionada amb la finestra de la aplicació. La classe hereda la clase Gtk.Window de gtk3
    ...
    def start_boxes(self)
        ...
        self.wM.create_box(Gtk.Orientation.HORIZONTAL)                                  #Creem la capsa 0 (inferior)
        self.wM.create_box(Gtk.Orientation.HORIZONTAL)                                  #Creem la capsa 1 (superior)
        self.wM.create_box(Gtk.Orientation.VERTICAL)                                    #Creem la capsa 2 (conté les capses 0 i 1)

        self.wM.add_widget_box(self.wM.boxes[2],self.wM.boxs[1],True,True,0)             #Afegim la capsa 1 (superior) a la capsa 2 (principal)
        self.wM.add_widget_box(self.wM.boxes[2],self.wM.boxs[0],False,False,0)           #Afegim la capsa 2 (superior) a la capsa 2 (principal)
...
class widgetManager:
    ...
    def create_box(self,orientació):
...
if __name__ == "__main__":
      finestra = Finestra(widgetManager())
      finestra.configure_window()
      Gtk.main()
