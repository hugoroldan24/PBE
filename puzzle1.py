...
Per instalarla, he fet servir la comanda:
pip install mfrc522-python 
Descarrego la llibreria desde el repositori en linea PyPI. Aquesta llibreria et fa interactuar amb el mòdul des de un nivell molt alt, però per l'aplicació demanada no és necessari programar-ho a un nivell més baix.
...
class Rfid_522:
  ...
  """ Llegueix el UID i ho guarda en format hexadecimal """
  def read_uid(self): 
      self.uid = hex(self.reader.read_id())[2:]  # Utilitzem el mètode read_id() de la llibreria SimpleMFRC522 que retorna l'id en format Integer. Després el convertim a hexadecimal i eliminem el prefixe "0x". 
